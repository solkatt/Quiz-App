...
function checkUserGuess() {
    ...
    state.gameplayState.stopTheGame = stopTheGame;
    turn = turn + 1;
    ...
    if (stopTheGame == false) {
        state.gameplayState.guessList.querySelectorAll('p').forEach(guess => {
            guess.remove();
        })
...
function displayOutput(player, guess, result) {
    ...
    switch (result) {
        ...
        case "lower":
            ...
            // display(player + "'s guess " + guess + " was too high! Try lower...");
            printBotGuess(player, guess, result);
            break;
        case "higher":
            minNumber = guess;
